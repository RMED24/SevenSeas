import { PUBLIC_TWITCH_CLIENT_ID, PUBLIC_TWITCH_CLIENT_SECRET } from '$env/static/public';
...
export class IGDB {
	private clientId: string = PUBLIC_TWITCH_CLIENT_ID ?? '';
	private clientSecret: string = PUBLIC_TWITCH_CLIENT_SECRET ?? '';
	...
	checkAndRenewToken = async () =>
		!!(Date.now() >= this.tokenExpiration - 100) && (await this.getAccessToken());
